...
- **goodmovies**: 흰. **(2026-07-29 개편 — 레이아웃은 레디액션식, 색만 굿무비 유지.)** 상 480 / 영상 960 / 하 480. 상단은 **시리즈 고정 카피**(default `死ぬまでに観たい**名作映画**\n**1000本を、**順不同で紹介中`, 50pt/200 검정, **만 500 — 영상별 멘트 아님, 매번 묻지 말 것). 자막 **영상 세로 중앙**: 원어 36px italic NSJP 흰(검정 stroke, 영어 음원만) 위 → 번역 48px/400 **노랑#FFEB3B**(검정 stroke) 아래. 하단 `#번호`(40px italic 검정) + `映画『제목』`(38px/200 검정 0.72) — **`#번호` = 1000 − 영상번호**(예 090 → `#910`, `new-video` 자동 주입).
  ...
  - **⚠️ 입력이 「레퍼런스 영상 + 원본 영상」 2개로 올 때**(2026-09-04~ 표준): 컷편집·오디오·SRT를 레퍼런스에서 가져오는 앞단 3스텝이 붙는다 → 아래 **「🎬 굿무비 레퍼런스 매칭」** 절 참조.
...
## 🎬 굿무비 레퍼런스 매칭 (레퍼런스 + 원본 2개 입력, goodmovies 전용)

**2026-09-04 확정 — 사용자 지시.** 굿무비는 앞으로 영상을 **2개** 받는다.

| 입력 | 정체 | 여기서 가져오는 것 |
| --- | --- | --- |
| **레퍼런스 영상** | 이미 컷편집된 짧은 클립 (영문 자막이 화면에 박혀 있음) | ① **영문 자막 + 타임스탬프**(= 화면에 박힌 자막 기준 SRT) ② **오디오 트랙 전체** ③ **컷 구성**(어디서 어디까지 잘라 붙였는지) |
| **원본 영상** | 원본 화질의 영화/드라마 소스 | **영상(픽셀)만** |

즉 결재본은 **"원본 화질의 그림 + 레퍼런스의 소리·자막·편집"**.

### 작업 순서

**`tools/ref-build.mjs` 가 R-1~R-4 를 한 번에 돈다.** 기본은 R-1 에서 멈추고 검증 시트를 내놓는다 —
컷 리스트는 **사람이 눈으로 확인하고 승인한 뒤에** 자른다는 게 원칙이라 그렇다.
...
node tools/ref-build.mjs "…레퍼런스.mp4" "…원본.mp4" --number 104 --scaffold \
  --slug terminator2-judgment-day --media-title-ja "ターミネーター2"
```

단계별로 따로 돌릴 수도 있다:

```
R-1  match-cuts     레퍼런스 컷 ↔ 원본 컷을 **프레임 단위**로 짝지어 오프셋 산출
                    (+ 오디오 오프셋을 따로 재서 A/V 어긋남 진단, 검증 시트 생성)
R-2b match-reframe  레퍼런스가 샷마다 쓴 크롭을 역산 → 채널 밴드 비율로 변환
R-3  ocr-subs       레퍼런스에 박힌 자막 줄을 자동 검출 + macOS Vision OCR → SRT
R-2  apply-cuts     원본을 컷대로 자르고 리프레임을 굽고 레퍼런스 오디오를 얹고 SRT 리매핑
R-4  기존 굿무비 흐름 그대로:
     prep-media → new-video → SRT cue 1:1 로 captions(경로 C) → 번역 →
     check-captions → 프리뷰 승인 → derive-lang → render → 캡션 txt
```

### 규칙

- **컷 지점은 자동 산출한다 — 눈대중/추측 금지. 다만 오디오가 아니라 화면이 1순위다.**
  레퍼런스 컷 시각과 원본 컷 시각을 **프레임 단위로 짝지어** 오프셋을 구한다. 컷은 불연속이라 변별력이 압도적이고,
  레퍼런스에 음악이 덧입혀져도 영향받지 않는다. 오디오 교차상관은 보조/교차검증용.
  **산출한 컷 리스트는 자르기 전에 검증 시트(`--verify`)를 보고 사용자 승인을 받을 것** — 한 컷이라도 어긋나면 뒤 공정이 전부 무의미해진다.
- **⚠️ 레퍼런스는 "컷을 붙여넣은 것"이 아니라 그냥 **한 구간을 통으로 자른 것**인 경우가 많다.**
  104(터미네이터2)가 그랬다 — 레퍼런스 컷 4개가 전부 원본 컷과 일치했다(= 영화 자체 컷). 이때 세그먼트는 1개다.
  `match-cuts` 는 레퍼런스 컷이 원본 컷과 맞으면 **그 앞뒤 구간을 같은 오프셋으로 묶는다**.
- **⚠️ 레퍼런스의 A/V 싱크가 어긋나 있을 수 있다 — 화면과 오디오 오프셋을 반드시 따로 재고 비교할 것.**
  104 에서 화면 76.449 / 오디오 76.666 → **레퍼런스 오디오가 그림보다 0.22초 앞서 있었다.**
  · 컷과 자막은 **화면 오프셋** 기준으로 간다 (박힌 자막은 그림의 일부고, 편집자가 컷에 스냅한다 —
    104 에서 자막 종료 2개가 컷 프레임과 정확히 일치해 이게 확인됐다).
  · 레퍼런스 오디오를 쓸 때 `apply-cuts --audio-shift <avSkew>` 를 주면 그 어긋남만 보정한다.
    **기본값 0 = 레퍼런스를 그대로 재현**(어긋남까지 그대로). 어느 쪽이든 사용자에게 알리고 고르게 할 것.
- **레퍼런스 마지막 몇 초에 음악이 덧입혀져 있을 수 있다** (104: 끝 4.5초, RMS 가 원본의 5~7배).
  그 구간은 오디오 매칭이 실패하니 화면으로 잡는다. 레퍼런스 오디오를 쓰면 그 음악도 같이 딸려온다.
- **최종 오디오는 레퍼런스 오디오 그대로.** 원본 오디오는 컷 지점을 찾는 "지문"으로만 쓰고 버린다. 이래야 SRT 타임스탬프와 100% 일치한다.
- **SRT 는 화면에 박힌 영문 자막에서 뽑는다 — 오디오 STT 로 만들지 말 것.** whisper 는 타이밍도 표기도 화면 자막과 다르다. 자막 밴드의 픽셀 변화로 cue 경계(등장/사라짐 초)를 잡고, 텍스트는 macOS Vision OCR(이 머신엔 tesseract 없음) 또는 샘플 프레임을 직접 읽어 확정한다. **cue 는 절대 묶지 않는다**(경로 C 불변식 — cue 수 = caption 수, 타임스탬프 무보정).
- **자막은 듀얼(원어+번역)이 기본** — 화면에 남는 건 **원본 픽셀**이고 원본엔 자막이 안 박혀 있으니 겹칠 게 없다. OCR 로 뜬 영문을 `original` 에 그대로 넣고 그 아래 노란 번역 → 흰 이탤릭 / 노랑 2단. (레퍼런스에 박혀 있던 자막은 레퍼런스 픽셀과 함께 버려진다.) **예외:** 원본에도 영문이 박혀 있는 클립이면 그때만 `original: ""`.
- **샷별 리프레임은 레퍼런스에서 역산한다 — 가운데 크롭 금지.**
  원본 2.4:1 → 밴드 1.125:1 이라 가로를 절반 넘게 버려야 하는데, 기계적 center crop 은 말하는 얼굴을 자른다.
  레퍼런스 편집자가 이미 샷마다 답을 냈으니 `match-reframe.mjs` 로 그 크롭을 되찾아 그대로 따른다
  (104: 줌 1.82배 + 샷마다 가로 위치만 이동). 결과는 시간분기 `crop` 으로 **미디어에 구워 넣는다** —
  밴드 크기(1080×960)까지 맞춰 두면 Remotion 의 cover 크롭이 무손실 통과가 된다.
- **모든 타임스탬프의 기준점은 컷편집본의 0초**(= 레퍼런스의 0초). 원본 타임코드가 아니다.
- **검정/단색 여백 크롭은 원본 쪽에, 컷편집 전에** 처리한다. 판단 기준은 기존 굿무비 크롭 규칙 그대로.

### 도구

| 도구 | 역할 |
| --- | --- |
| `tools/ref-build.mjs` | **오케스트레이터** — R-1~R-4 를 한 번에. 기본은 R-1 에서 멈추고 검증 시트를 낸다(`--go` / `--scaffold`) |
| `tools/match-cuts.mjs` | 컷 매칭 — 화면(프레임 단위 컷 짝짓기) 1순위 + 오디오 보조. 레퍼런스/원본 크롭 자동검출, **A/V 어긋남 진단**, `--verify` 검증 시트 |
| `tools/match-reframe.mjs` | 레퍼런스가 샷마다 쓴 크롭을 역산 → 채널 밴드 비율로 변환 + 시간분기 `crop` 식 |
| `tools/apply-cuts.mjs` | 컷 적용 + 리프레임 굽기 + 오디오(`--audio ref\|orig`, `--audio-shift`) + **SRT 리매핑** |
| `tools/ocr-subs.mjs` | 자막 줄 자동검출(원어/번역 2줄이면 `--band` 로 선택) + macOS Vision OCR → SRT |
| `tools/vidutil.mjs` | 위 도구들이 공유하는 저수준 유틸 (프레임 단위 컷 검출, 파형 오프셋, 회색 프레임 디코드) |

### ⚠️ ffmpeg 함정 3개 (전부 104 에서 밟았다 — `vidutil.mjs` 가 막아준다)

1. **`crop` 은 짝수여야 한다.** 홀수를 주면 ffmpeg 가 조용히 내림해서 rawvideo 프레임 크기가 어긋나고,
   모든 타임스탬프가 **비율로** 드리프트한다(104: 4%). `grayFrames` 가 짝수를 강제하고 바이트 정렬을 검사한다.
2. **`-fps_mode passthrough` 없이 raw 디코드하면 프레임이 하나 늘어난다.** `start_time` 이 0 이 아닌 파일
   (예: 첫 프레임 pts=1001)에서 ffmpeg 가 t=0 을 메우려고 **맨 앞 프레임을 복제**한다 → 프레임 인덱스가 통째로 +1 밀린다.
3. **프레임을 정확히 집을 땐 `-ss <초>` 대신 프레임 번호(`select=eq(n,N)`)를 쓴다.** `-ss` 는 "그 시각 **이상**의 첫 프레임"을
   주므로 초를 반올림해 넘기면 한 프레임 밀린다. 검증 시트가 이것 때문에 한 번 잘못 나왔다.

그리고 **압축 오디오를 세그먼트마다 AAC 로 인코딩하지 말 것** — 인코더 프라이밍(1024샘플 ≈ 21ms)이 매번 붙어
화면보다 그만큼 앞당겨진다. `apply-cuts` 는 PCM 으로 잘라 이어 붙이고 **맨 끝에 한 번만** AAC 로 인코딩한다.
...
### 표준 워크플로 (2026-08-31 확정 — 사용자 지시)

```
사용자가 레퍼런스 유튜브 링크를 줌
  → Claude 가 댓글 수집 + 리스트업          (fetch-comments.mjs)
  → 사용자가 쓸 댓글을 고름
  → Claude 가 렌더                          (render-comment.mjs --scale 1 --soft2x)
  → 댓글 폴더 생성 후 편집에 얹음            (prep-comments.mjs → props.comments)
```

- **리스트업 분량:** 영상 길이 ÷ 6초 ≈ 필요 개수. 고를 수 있게 **그 2배쯤** 뽑아 준다 (56초 → 9~10개 필요 → 20개 리스트업).
- **렌더 뒤 반드시 검수:** `node tools/verify-comments.mjs <번호>댓글` — 코퍼스 대조 표 + manifest 정합성 + **`_검수시트.png`**(전 장을 라벨과 함께 한 장으로). Claude 는 그 시트를 직접 열어 눈으로 확인한 뒤 다음 단계로 간다. 숫자만 믿으면 안 된다(과거 오류가 전부 "좌표는 ✓"였다).
- **폴더에 남기는 것:** `_수집.json`(고른 댓글 원본 데이터 — 나중에 재렌더/수정 가능), `manifest.json`(파일명·note·블러 좌표), `_검수시트.png`. `_` 로 시작하는 파일은 댓글 스샷이 아니므로 도구들이 건너뛴다.
- 고르는 기준(아키타입·U자 곡선·배합)은 [tools/jp-comment-picker.md](./tools/jp-comment-picker.md).
- 렌더 파일명은 **댓글 내용의 한국어 번역** — 그게 곧 그 댓글의 제목/식별자다(화면엔 미표시).
- 사용자가 직접 찍은 스샷을 주는 경로도 그대로 살아 있다(아래). 두 경로가 한 폴더에 섞여도 된다.

**입력 (사용자가 스샷을 직접 줄 때):** 레포 루트에 **`<번호>댓글/`** 폴더 (영상 소스처럼 최상단). 그 안에 댓글 스샷들(png). 파일명은 아무거나(스크린샷 원본명).
...
**⚠️ 합성 렌더(`render-comment.mjs`)의 규격은 눈대중이 아니라 채택본 전수 실측에서 나온다 (2026-08-31 교정).**
스샷 한 장에 맞추면 다음 장이 틀어진다 — 스샷마다 브라우저 줌·레티나 배율이 섞여 있어서다.
그래서 `measure-comments.mjs` 가 채택본 107장을 재고, **줄높이(lineH)로 나눈 비율**(배율 불변)로 규격을 잡는다.
렌더를 고쳤으면 반드시 `node tools/measure-comments.mjs --compare <렌더.png>` 로 코퍼스와 대조해 전 항목 ✓ 를 확인할 것.

- **배경은 `#111111`** — 유튜브 CSS 명세는 `#0f0f0f` 지만 실제 맥 스샷은 컬러 프로파일을 거쳐 `#111111` 로 찍힌다(107장 중 69장). 명세값을 쓰면 안 된다.
- **⚠️ 엄지 아이콘은 그리지 말고 오려 쓴다.** `tools/assets/yt-thumb-{up,down}.png` — 실제 스샷에서 잉크 bbox 15×16 으로 크롭한 **진짜 픽셀**이다. 머티리얼 3종(Icons Outlined / Symbols Outlined / Symbols Rounded)은 전부 소매가 손과 분리돼 있어 아예 다르고, 직접 트레이스한 패스도 곡률·기울기가 미묘하게 어긋나 **"티가 난다"**(사용자 지적). 유튜브 아이콘은 어느 댓글이든 동일하므로 한 번 오려 두면 영원히 정확하다.
  - **오려낸 건 엄지 그림 2개뿐** — 좋아요 수·핸들·날짜·본문은 전부 수집 데이터로 그린다(숫자는 아이콘 옆 텍스트).
  - 1x 비트맵이라 **반드시 `--scale 1 --soft2x`** 로 뽑을 것. 네이티브 2x 로 그리면 아이콘만 뭉갠다.
- **스레드 세로선을 그린다 (기본 켬).** 답글 달린 댓글에 유튜브가 프사 중심 x 에 1px `#373737` 선을 아래로 긋는다. **채택본 92장 중 72장**에 있어서, 없으면 오히려 합성 티가 난다. 끄려면 `--no-thread-line`.
- **프사는 실사진을 쓴다.** `fetch-comments.mjs` 가 innertube 의 `avatarThumbnailUrl` 을 같이 담아 오고, 렌더가 data URI 로 박는다. 단색 원 + 이니셜 폴백은 합성 티가 나서 하단 밴드에서 이질적으로 보인다(사용자 지적).
- **액션행 내부 간격은 균등하지 않다.** 좋아요수→싫어요 18px, 싫어요→返信 29px (실측). 균등하게 주면 총 폭은 맞는데 싫어요 아이콘만 8px 밀린다 — 숫자로는 안 잡히고 **겹쳐 봐야** 드러난다.
- **`-webkit-font-smoothing:antialiased` 를 켜지 말 것.** 글자가 가늘어져 실제 스샷보다 잉크가 20% 적고 흐리다(1363 vs 1665). 맥 크롬 기본값이 맞다.
- **⚠️ 네이티브 2x 로 그리지 말고 `--soft2x`(1x 렌더 → lanczos 2x)를 쓸 것.** 사용자 스샷은 1x 라 prep 단계에서 2x 로 늘려 쓰는데, 합성 렌더만 네이티브 2x 로 뽑으면 가장자리가 혼자 쨍해서 같은 영상 안에서 튄다. **같은 경로를 태워야 질감이 맞는다.**
- **⚠️ 좋아요 수는 흰색이 아니라 회색이다.** 흰색(#f1f1f1)으로 찍으면 숫자만 유독 진해서 바로 티가 난다 — 실측 최대밝기 **실제 160 vs 흰색 241**. 날짜와 같은 보조색 `#a0a0a0`(#aaa 도 아직 밝다). **핸들·返信 은 흰색이되 굵기 400** (500 이면 실제보다 진하다).
- 실측 규격(1x 기준): 프사 ø36 / 프사↔텍스트 16 / textLeft 58 / 핸들 12.5px·500 / 본문 14px·line-height 20 / 액션 12px / 아이콘 20px.
- **검수는 숫자 + 겹쳐보기 둘 다.** 배율 맞춘 뒤 `ffmpeg -i 실제.png -i 렌더.png -filter_complex "[0:v]format=gray[a];[1:v]format=gray[b];[a][b]mergeplanes=0x001000:gbrp"` — 노랑=실제만, 파랑=렌더만, 흰색=일치.
...
| `check-caption-txt.mjs` | 결재본은 있는데 `<번호>캡션.txt` 가 빠진 폴더 탐지 — **렌더 뒤 목록이 비어야 완료** |
...
| `verify-comments.mjs` | 댓글 폴더 검수 — 코퍼스 대조 표 + manifest 정합성 + `_검수시트.png`(눈으로 볼 한 장) + `--ref` 오버레이 (굿바이브 전용) |
| `measure-comments.mjs` | 채택 스샷 **전수 실측** → 렌더 규격을 비율 통계로 산출. `--compare <png>` 로 내 렌더를 코퍼스와 대조 (굿바이브 전용) |
...
| `ref-build.mjs` | 🎬 굿무비 레퍼런스 워크플로 오케스트레이터 (R-1~R-4). 「🎬 굿무비 레퍼런스 매칭」 절 참조 |
| `match-cuts.mjs` | 🎬 레퍼런스↔원본 컷 매칭 + A/V 어긋남 진단 + 검증 시트 |
| `match-reframe.mjs` | 🎬 레퍼런스의 샷별 크롭 역산 → 밴드 비율 변환 |
